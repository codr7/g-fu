...
While derived environments capture used bindings; except for `do`, `let` and `set` which are always included.

```
  (let _ (this-env))

(do:(prim do) let:(prim let) set:(fun set (args..) n/a) this-env:(fun this-env () n/a))
```
